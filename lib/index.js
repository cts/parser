...
var parseSheetSelector = function(selectorString) {
  ...
  if (match = CTS.Parser._rTree.exec(s)) {
    ret.tree = match[1];
    s = match[2];
  }

  // WORKSHEET PREFIX
  if (match = CTS.Parser._rWorksheet.exec(s)) {
    // There's a worksheet string
    ret.worksheet = match[1];
    s = match[2];
  }

  if (match = CTS.Parser._rRows.exec(s)) {
    ret.projection = 'Rows';
  } else if (match = CTS.Parser._rCols.exec(s)) {
    ret.projection = 'Cols';
  } else if (match = CTS.Parser._rRow.exec(s)) {
    ret.projection = "Rows";
    ret.row = match[1];
  } else if (match = CTS.Parser._rCol.exec(s)) {
    ret.projection = "Cols";
    ret.col = match[1];
  } else if (match = CTS.Parser._rCell.exec(s)) {
